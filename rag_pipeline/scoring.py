...
# Concepts dont les implications sont des COMPOSANTS (parties d'un tout)
# et non des SOUS-TYPES (spécialisations). Pour ceux-ci, le match child
# reste dégressif (90/80/70%) au lieu de 100%.
# Ex : "Rythme sinusal" est un composant d'ECG normal, pas un sous-type.
COMPONENT_PARENT_IDS = {"ECG_NORMAL"}
    ...
    **Score = moyenne des % des diagnostics VALIDANTS matchés.**
    ...
    for gname, gid in zip(golden_names, golden_ids):
        ...
        for fid in found_id_set:
            ...
            #     L'enfant est PLUS PRÉCIS que le golden → 100% (ou 80% si hypothèse)
            #     SAUF si le golden est dans COMPONENT_PARENT_IDS (ex: ECG_NORMAL)
            #     où les enfants sont des composants, pas des sous-types → dégressif
            #     GARDE-FOU : un simple DESCRIPTEUR_ECG (poids=1) ne peut pas valider
            #     un diagnostic (poids≥2). Ex: "bradycardie" ne valide pas "BAV de haut grade".
            #     Ex OK  : golden="Flutter droit typique", étudiant="Flutter antihoraire" → 100%
            #     Ex NOK : golden="ECG normal", étudiant="Rythme sinusal" → 90% (composant)
            #     Ex NOK : golden="BAV haut grade", étudiant="Bradycardie" → rejeté (descripteur)
            if fid in golden_child_depths:
                # Garde-fou : rejeter si le concept trouvé est un simple descripteur (poids=1)
                # et que le golden est un diagnostic (poids≥2)
                fid_owl = concept_mappings.get(fid, {})
                fid_poids = fid_owl.get("poids", 1)
                golden_poids = owl_golden.get("poids", 1) if owl_golden else 1
                if fid_poids < 2 and golden_poids >= 3:
                    # Un descripteur ne peut pas valider un diagnostic
                    logger.info(
                        f"   ⛔ CHILD rejeté : {fid} (poids={fid_poids}) trop faible pour {gname} (poids={golden_poids})"
                    )
                    continue  # Passer au fid suivant

                generation = golden_child_depths[fid]
                if gid in COMPONENT_PARENT_IDS:
                    base_score = _score_for_generation(generation)  # dégressif
                else:
                    base_score = 100.0  # sous-type = score plein
